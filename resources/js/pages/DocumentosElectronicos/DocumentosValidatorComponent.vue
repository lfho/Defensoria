 ...
 */
@Component
export default class DocumentosValidatorComponent extends Vue {
     ...
     */
     public validar_documento_electronico(codigo: any): void {
        ...
        axios.post('validar-documento-electronico-codigo/'+codigo)
        .then((res) => {
            ...
            if(dataDecrypted["data"].length) {
                ...
                this.$swal({
                    icon: "warning",
                    html: "Lamentablemente, el código que ingresó no concuerda con un documento generado por la Intraweb. Por favor, compruebe el código que ingresó para asegurarse de que es correcto o póngase en contacto con el servicio de atención al cliente para obtener más información.",
        ...
        .catch((err) => {
            ...
            (this.$parent as any)._pushNotification('Error al leer documento'+err, false, 'Error');
        });
    }
     ...
     * @version 1.0.0
     */
     public validar_adjunto_documento(): void {
        // Valida que el campo del código de validación venga vacío
        ...
        axios
            .post("validar-adjunto-documento-electronico", this.makeFormData(), {
            ...
            .then(res => {
                ...
                if (dataDecrypted["data"]["documentos_identicos"]) {
                    // Mensaje para indicarle al usuario que el documento fue verificada y validada
                    this.$swal({
                        icon: "success",
                    ...
                    // Muestra un mensaje al usuario indicándole que no encontró un documento según el código ingresado
                    this.$swal({
                        icon: "warning",
                        html: "Lamentablemente, el documento adjunto no coincide con el documento almacenado, el documento adjunto tiene un contenido diferente al del documento almacenado. Por favor, comprueba el documento adjunto para asegurarte de que es el correcto",
                        allowOutsideClick: false,
                        confirmButtonText: this.lang.get("trans.Accept")
                    });
                }
